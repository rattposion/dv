...
export default function Inventario() {
  ...
  const { caixas: supabaseCaixas, loading } = useSupabaseCaixasInventario();
  const { caixas, syncWithSupabase } = useInventario();
  
  // Sincronizar inventário com estoque
  useInventarioEstoqueSync();
  
  // Sincronizar dados do Supabase com o contexto local
  useEffect(() => {
    if (supabaseCaixas && supabaseCaixas.length > 0) {
      syncWithSupabase(supabaseCaixas);
    }
  }, [supabaseCaixas, syncWithSupabase]);
