...
export default function Recebimentos() {
  ...
  const salvarRecebimento = async () => {
    if (!novoRecebimento.numero_documento || !novoRecebimento.origem || 
    ...
    try {
      ...
      const { data: recebimentoData, error: recebimentoError } = await supabase
        .from('recebimentos')
        .insert([{
          numero_documento: novoRecebimento.numero_documento,
  ...
  return (
    <DashboardLayout>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNovoRecebimento(prev => ({ ...prev, numero_documento: e.target.value }))}
                ...
                <div className="grid grid-cols-4 gap-4">
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        // Buscar equipamento selecionado para preencher modelo automaticamente
                        const equipamentoSelecionado = equipamentos.find(eq => eq.nome === value);
                        setNovoItem(prev => ({ 
                          ...prev, 
                          produto_nome: value,
                          modelo: equipamentoSelecionado?.modelo || prev.modelo
                        }));
                      }}
                    >
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        {equipamentos.map((equipamento) => (
                          <SelectItem key={equipamento.id} value={equipamento.nome}>
                            {equipamento.nome} {equipamento.marca && `- ${equipamento.marca}`}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
      ...
      {/* Melhorar o Card de Estoque de Recebimento */}
