-- =====================================================
-- MIGRAÇÃO COMPLETA DO SISTEMA DE CHECKLIST
-- Data: 29/01/2025
-- Descrição: Criação e otimização completa do sistema de checklists
-- =====================================================
...
-- Remover tabela existente se houver (apenas para desenvolvimento)
-- DROP TABLE IF EXISTS public.checklists CASCADE;

-- Criar tabela de checklists com estrutura completa
...
-- Índices para busca por equipamento
...
RETURNS TRIGGER AS $$
DECLARE
    total_tests INTEGER := 5; -- wifi, lan, login, medicao, velocidade, dados
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'checklists') THEN
        RAISE NOTICE 'Tabela checklists criada com sucesso!';
    ELSE
        RAISE EXCEPTION 'Erro ao criar tabela checklists';
    END IF;
END $$;